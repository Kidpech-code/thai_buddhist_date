...
String format(DateTime dt, {String format = 'yyyy-MM-dd', Era era = Era.be, String? locale}) {
  if (era == Era.ce) {
    return (locale == null || locale.isEmpty) ? DateFormat(format).format(dt) : DateFormat(format, locale).format(dt);
  }
...
class ThaiCalendar {
  ...
  static Future<void> ensureInitialized([String? locale, ThaiLanguage? language]) {
    final loc = language != null ? _languageToLocale(language) : (locale?.isNotEmpty == true ? locale! : _defaultLocale);
  ...
  static String formatWith(DateFormat df, DateTime date, {Era era = Era.be}) {
    ...
    if (era == Era.ce) {
      return (loc.isEmpty) ? DateFormat(pattern).format(date) : DateFormat(pattern, loc).format(date);
    }
  ...
  static DateTime? parse(String input, {String? customPattern, String? locale, ThaiLanguage? language}) {
    input = input.trim();
    final loc = language != null ? _languageToLocale(language) : (locale?.isNotEmpty == true ? locale! : ThaiDateSettings.defaultLocale);
...
class ThaiFormatter {
  ...
  String formatDate(DateTime date) => tbdFormat(date, pattern: pattern, era: era, locale: _resolveLocale());

  String _resolveLocale() {
    if (language != null) return _languageToLocale(language!);
    return (locale?.isNotEmpty == true) ? locale! : ThaiDateSettings.defaultLocale;
  }
}

// Internal alias to avoid shadowing the top-level function name inside class methods.
String tbdFormat(DateTime dt, {required String pattern, required Era era, String? locale}) => format(dt, format: pattern, era: era, locale: locale);

/// DateTime extensions for ergonomic usage.
extension ThaiDateTimeFormatting on DateTime {
  String toThaiString({String pattern = 'yyyy-MM-dd', Era? era, String? locale, ThaiLanguage? language}) {
    final resolvedLocale = language != null ? _languageToLocale(language) : (locale?.isNotEmpty == true ? locale! : ThaiDateSettings.defaultLocale);
    return format(this, format: pattern, era: era ?? ThaiDateSettings.defaultEra, locale: resolvedLocale);
  }
}

/// String parsing extensions.
extension ThaiStringParsing on String {
  DateTime? toThaiDate({String? pattern, String? locale, ThaiLanguage? language}) {
    return ThaiCalendar.parse(this, customPattern: pattern, locale: locale, language: language);
  }
}
