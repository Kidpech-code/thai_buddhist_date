...
// ------------------------
// DateTime Picker (+ preview format)
// ------------------------
...
// Public helpers for single pickers
...
// Bottom Sheet (single)
// ------------------------
Future<DateTime?> showThaiDatePickerBottomSheet(
  BuildContext context, {
  DateTime? initialDate,
  DateTime? firstDate,
  DateTime? lastDate,
  ...
  final res = await showModalBottomSheet<DateTime?>(
    ...
    clipBehavior: sheetClipBehavior,
    builder: (sheetContext) {
      final screenH = MediaQuery.of(sheetContext).size.height;
      double calHeight = height ?? screenH * 0.6;
      if (calHeight < 320) calHeight = 320;
                ...
                children: [
                  Expanded(
                    child: BuddhistGregorianCalendar(
                      era: era,
                      locale: locale,
                      initialMonth: (selected ?? initialDate) ?? DateTime.now(),
                      selectedDate: selected,
                      firstDate: firstDate,
                      lastDate: lastDate,
                      headerBuilder: headerBuilder,
                      dayBuilder: dayBuilder,
                      onDateSelected: (d) => setModalState(() => selected = d),
                    ),
                  ),
                  Row(
...
// ------------------------
// Range picker (returns DateTimeRange?)
// ------------------------
...
// ------------------------
// Multi-date picker (returns Set<DateTime>)
// ------------------------
